...
# 3. 雲端資料庫讀寫核心 (採用 Streamlit 官方 GSheets 連結器)
...
def save_cloud_data(user_name, user_answers):
    try:
        # 1. 先讀取目前雲端上「所有人」的完整資料
        current_cloud = load_cloud_data()
        
        # 2. 用目前這位使用者最新勾選的進度，覆蓋或新增進去
        current_cloud[user_name] = user_answers
        
        # 3. 把所有的資料重新整理成 DataFrame 表格格式
        rows = []
        ...
        # 4. 真正寫入 Google Sheets 雲端
        conn.update(spreadsheet=GOOGLE_SHEET_URL, data=df_new)
        return True
    except Exception as e:
        st.error(f"寫入雲端失敗，錯誤訊息: {e}")
        return False

# 4. 核心 CSS 注入（維持淺色舒適毛玻璃風格）
custom_style = """
<style>
...
with st.sidebar:
    ...
    if st.button("🔄 同步最新雲端進度", use_container_width=True):
        st.session_state.cloud_data = load_cloud_data()
        st.toast("⚡ 已手動同步最新雲端數據！")

# 6. ▶️ 中央主畫面大標題
st.markdown('<div class="hero-card"><h1 style="margin:0; font-size:1.8rem; font-weight:700; color:#1a365d !important;">🇦🇺 2026 澳洲自駕隨身手冊</h1></div>', unsafe_allow_html=True)
...
with tab3:
    ...
    st.write(f"請勾選 **{user_name}** 已放進行微或行李箱的物品：")
    ...
    if st.button("💾 儲存今日進度至雲端", type="primary", use_container_width=True):
        with st.spinner("正在安全同步至 Google 雲端試算表..."):
            ...
            if success:
                ...
                st.error("雲端儲存發生異常，請檢查表格共用編輯權限是否開啟。")
